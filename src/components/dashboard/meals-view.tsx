...
import { Brain, Clock, Flame, ChefHat, Sparkles, RefreshCw } from "lucide-react";
...
function generateMeals(): MealSuggestion[] {
  return [
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div className="flex items-center gap-3 px-4 py-3 bg-accent-2/5 border border-accent-2/20 rounded-xl flex-1">
          ...
          <p className="text-sm text-accent-2 font-medium"><span className="font-bold">{meals.length} meal ideas</span> generated from your current inventory</p>
        </div>
        <motion.button whileHover={{ rotate: 180 }} transition={{ duration: 0.3 }} className="ml-3 p-2.5 rounded-xl border border-card-border hover:border-accent-2/30 text-muted hover:text-accent-2 transition-all">
